...
import { TECHNICAL_FOCUS, TECHNICAL_FOCUS_INTRO } from '../../../src/data/technical-focus';
import { caseStudiesByTopic } from '../../../src/data/relations';
...
export default function TechnicalFocusPage() {
  ...
  const areas = TECHNICAL_FOCUS.map((area) => ({
    slug: area.slug,
    title: area.title,
    icon: area.icon,
    ...
    <>
      ...
      <Section>
        <p className="mb-6 text-sm text-ink-600">
          Select an area for its summary and the case studies behind it.
        </p>
        <FocusMosaic areas={areas} />
      </Section>

      <Section className="pt-0">
        <CrossLinks hrefs={['/services', '/distinctives', '/team']} />
      </Section>
    </>
  );
}
